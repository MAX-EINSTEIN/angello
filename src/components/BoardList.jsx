...
const BoardList = ({ logOut, boards, addNewBoard, deleteBoard, name }) => {
    ...
    if (navigator.onLine !== true) {
        ...
        <div className='bg-gradient-to-r from-indigo-50 via-blue-50 to-red-50 h-screen px-6 py-4 sm:py-20 sm:px-24'>
            ...
            <div className='flex flex-col my-2'>
                <div className='flex justify-between'>
                    <h1 className='text-xl sm:text-3xl bg-gradient-to-b from-secondary to-primary bg-clip-text text-transparent'>Welcome, {name ? name.split(' ')[0] : 'Stranger'}</h1>
                    <button className='px-2 py-0.5 bg-gradient-to-b from-primary to-secondary text-gray-100 rounded-sm text-sm sm:text-base' onClick={logOut}>Log out</button>
                </div>
                <div className="my-12">
                    <h1 className='text-xl text-tertiary mb-2'>Your Boards</h1>
            ...
            <form onSubmit={addNewBoard} autoComplete='off' className='my-4 sm:my-8'>
                <label htmlFor="boardName" className='block text-xl text-tertiary mb-2'>Make a new board</label>
                <div className="flex gap-1 items-center mt-2">
                    <input required type="text" name='boardName' className='bg-transparent border border-gray-500 px-2 py-1 text-lg rounded-sm placeholder-gray-700' placeholder='Enter a board name' />
                    <button type='submit' className='bg-gradient-to-b from-primary to-secondary text-gray-100  rounded-sm px-3 text-3xl font-bold font-mono'>+</button>
                </div>
            </form>
        </div>
    )
